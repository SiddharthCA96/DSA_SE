...
import { idf, tf_idf, Db_Keyword, Db_mag, all_problem } from "./backend/db/index.js";
...
mongoose.connect('mongodb+srv://ss6156852:ZCUwQ7HGO2u7IJjm@cluster0.sr1uz.mongodb.net/DSA_SE')
    ...
    async function saveCompressedTfIdf(tf_idf_matrix) {
        ...
        if (typeof tf_idf_matrix === 'string') {
            ...
            matrixArray = tf_idf_matrix.split('\n').map(row => row.split(' ').map(Number)); 
        } else {
...
async function processData() {
    try {
        // Reading files
        const all_keywds = (await fs.readFile('all_keywords.txt', 'utf-8')).toString();
        const idf_str = (await fs.readFile('idf_score.txt', 'utf-8')).toString();
        const tf_idf_matrix = (await fs.readFile('tf_idf_Matrix.txt', 'utf-8'));
        const problem_titles = (await fs.readFile('leetcode_prob_titles.txt', 'utf-8')).toString().split('\n');
        const problem_urls = (await fs.readFile('leetcode_prob_url.txt', 'utf-8')).toString().split('\n');
        const problem_desc = await Promise.all(
            Array.from({ length: 1737 }, async (_, i) => {
                try {
                    ...
                    console.error(`Error reading problem description for leet_prob${i + 1}:`, err);
                    return '';
                }
            })
        );
